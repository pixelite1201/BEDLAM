...
class SMPLXCamHeadProj(nn.Module):
    def __init__(self, focal_length=5000., img_res=224):
        ...
        self.downsample_mat_smplx = pickle.load(open(config.DOWNSAMPLE_MAT_SMPLX_PATH_12, 'rb')).cuda()
    ...
    def forward(self, rotmat, shape, cam, cam_intrinsics,
        ...
        downsample_verts = torch.einsum('bij,bjk->bik', self.downsample_mat_smplx.repeat(batch_size, 1, 1), output['vertices'])
