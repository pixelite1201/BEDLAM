...
class HMRXLoss(nn.Module):
    def __init__(
            self,
            hparams=None,
    ):
        super(HMRXLoss, self).__init__()
    ...
    def forward(self, pred, gt):
        ...
        pred_body_pose = pred['pred_pose']
        pred_lhand_pose = pred['pred_lhand_pose']
        pred_rhand_pose = pred['pred_rhand_pose']
        pred_rotmat = torch.cat((pred_body_pose, pred_lhand_pose, pred_rhand_pose), 1)
        ...
        gt_body_pose = gt['pose']
        gt_lhand_pose = gt['lhand_pose']
        gt_rhand_pose = gt['rhand_pose']
        gt_pose = torch.cat((gt_body_pose, gt_lhand_pose, gt_rhand_pose), 1)
...
def projected_keypoint_loss_oldv(
        pred_keypoints_2d,
        gt_keypoints_2d,
        criterion,
):
